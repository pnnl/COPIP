...
class SEGP(nn.Module):
    ...
    def __init__(self, m:int, n:int, p:int, lt:float, mean_x0:torch.tensor, covar_x0:torch.tensor,
        ...
        self.p = p
        self.lt = lt
        self.register_buffer('mean_x0', mean_x0.unsqueeze(0).t() ) # (n,1)
        self.register_buffer('covar_x0', covar_x0) # (n,n)
        ...
        if A==None:
            ...
            parametrize.register_parametrization(self.V2, "weight", L_plus(n))
    ...
    def get_AP(self):
        ...
        # for testing issue with V1_inv error - remove try-except block once fixed!!
        try:
            V1_inv = torch.inverse( self.V1.weight + self.eps )
        except:
            print( self.V1.weight )
            V1_inv = torch.inverse( self.V1.weight + self.eps )
    ...
    def SE_kernel(self, T1:torch.tensor, T2:torch.tensor):
        ...
        returns:
            covar_U: SE Covariance matrix between T1 and T2 (N1, N2).
        """
    ...
    def Cholesky_inverse(self, covar):
        ...
        returns:
                covar_inv: Inverse of covar (bs, m*N, m*N).
        """
        L_inv = torch.inverse( torch.linalg.cholesky(covar) )
        covar_inv = torch.matmul( L_inv.transpose(1,2), L_inv )
    ...
    def posterior(self, dT1:torch.tensor, dT2:torch.tensor, tmax1:float, tmax2:float, mean_U1:torch.tensor, mean_U2:torch.tensor,
        ...
        try:
              # saves recomputing if they have already been computed in forward.
              mean_dT1 = self.mean_prior # shape = (N1, m)
              covar_dT1 = self.covar_prior # shape = (m*N1, m*N1)
        except:
        ...
        self.register_buffer('eps', 1e-3 * torch.eye(self.m*N1, self.m*N1).repeat(bs,1,1), persistent=False )
        inv = self.Cholesky_inverse( covar_dT1.repeat(bs, 1, 1)  + covar_noise + self.eps ) # (bs, m*N1, m*N1)

        self.mean_post = (obs - mean_dT1.repeat(bs, 1, 1)).transpose(1, 2).flatten(start_dim=1, end_dim=2).unsqueeze(2) # shape = (bs, m*N1, 1)
        self.mean_post = torch.bmm(inv, self.mean_post) # shape = (bs, m*N1, 1)
        self.mean_post = mean_dT2.repeat(bs,1,1) + torch.bmm( covar_dT2_dT1.repeat(bs,1,1), self.mean_post ).squeeze(2).unflatten( dim=1, sizes=(self.m, N2) ).transpose(1,2) # shape = (bs, N2, m)

        self.covar_post = torch.bmm( inv, covar_dT1_dT2.repeat(bs,1,1) ) # (bs, m*N1, m*N2)
        self.covar_post = covar_dT2 - torch.bmm( covar_dT2_dT1.repeat(bs,1,1), self.covar_post ) # (bs, m*N2, m*N2)
