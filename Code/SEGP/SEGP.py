...
class Non_negative(nn.Module):
    """
    For mapping unconstrained matrix to one with non-negative elements.
    """
    def forward(self, Z):
        return torch.abs(Z)
...
class SEGP(nn.Module):
    ...
    def __init__(self, m:int, n:int, p:int, lt:float, mean_x0:torch.tensor, covar_x0:torch.tensor,
        ...
        if B==None:
            ...
            parametrize.register_parametrization(self.B, "weight", Non_negative() )
        else:
            self.B_ind = False
            self.B = B
        ...
        if D==None:
            ...
            parametrize.register_parametrization(self.D, "weight", Non_negative() )
        else:
            self.D_ind = False
            self.D = D
    ...
    def mean_module(self, T:torch.tensor, dT:torch.tensor, tmax:float, mean_U:torch.tensor, mean_dU:torch.tensor,
                    A:torch.tensor, B:torch.tensor, C:torch.tensor, D:torch.tensor):
        """
    ...
    def covar_module(self, T1:torch.tensor, T2:torch.tensor, dT1:torch.tensor, dT2:torch.tensor, tmax1:float, tmax2:float, mean_U1:torch.tensor,
        ...
        term2 = torch.stack(term2) # (N1, N2, m, m)
        ...
        self.register_buffer('eps', 1e-5 * torch.eye(self.m*N1, self.m*N2), persistent=False)
    ...
    def posterior(self, covar_lhood, mean_lhood, mean_prior, covar_prior):
        """
        ...
        L = torch.linalg.cholesky(covar_prior.repeat(bs,1,1) + covar_lhood, upper=False) # shape = (bs, m*N, m*N)

        mean_post = (mean_lhood - mean_prior.repeat(bs, 1, 1)).transpose(1, 2).flatten(start_dim=1, end_dim=2).unsqueeze(2) # shape = (bs, m*N, 1)
        mean_post = torch.cholesky_solve(mean_post, L ) # shape = (bs , m*N, 1)    
        mean_post = torch.bmm(covar_prior.repeat(bs,1,1), mean_post ).squeeze(2).unflatten(dim=1, sizes=(m, N)).transpose(1,2) # shape = (bs, N, m)
        mean_post = mean_prior.repeat(bs,1,1) + mean_post # shape = (bs, N, m)

        covar_post = torch.cholesky_solve( covar_prior.repeat(bs,1,1) , L) # shape = (bs, m*N, m*N)
        covar_post = covar_prior.repeat(bs,1,1) - torch.bmm( covar_prior.repeat(bs,1,1), covar_post ) # (bs, m*N, m*N)

        return mean_post, covar_post
    
    
    
    def predictive(self, T1, T2, dT1, dT2, tmax1, tmax2, mean_U1, mean_U2, mean_dU1, mean_dU2, covar_noise, obs):
        """
        ...
        mean_dT1 = self.mean_module(T1, dT1, tmax1, mean_U1, mean_dU1, A, B, C, D) # shape = (N1, m)
        covar_dT1 = self.covar_module(T1, T1, dT1, dT1, tmax1, tmax1, mean_U1, mean_U1, mean_dU1, mean_dU1, A, B, C, D) # shape = (m*N1, m*N1)

        mean_dT2 = self.mean_module(T2, dT2, tmax2, mean_U2, mean_dU2, A, B, C, D) # shape = (N2, m)
        covar_dT2 = self.covar_module(T2, T2, dT2, dT2, tmax2, tmax2, mean_U2, mean_U2, mean_dU2, mean_dU2, A, B, C, D) # shape = (m*N2, m*N2)
        covar_dT1_dT2 = self.covar_module(T1, T2, dT1, dT2, tmax1, tmax2, mean_U1, mean_U2, mean_dU1, mean_dU2, A, B, C, D) # shape = (m*N1, m*N2)
        ...
        self.mean_pred = (obs - mean_dT1.repeat(bs, 1, 1)).transpose(1, 2).flatten(start_dim=1, end_dim=2).unsqueeze(2) # shape = (bs, m*N1, 1)
        self.mean_pred = torch.cholesky_solve(self.mean_pred, L ) # shape = (bs , m*N1, 1)    
        self.mean_pred = torch.bmm(covar_dT2_dT1.repeat(bs,1,1), self.mean_pred ).squeeze(2).unflatten(dim=1, sizes=(self.m, N2)).transpose(1,2) # shape = (bs, N2, m)
        self.mean_pred = mean_dT2.repeat(bs,1,1) + self.mean_pred # shape = (bs, N2, m)

        self.covar_pred = torch.cholesky_solve( covar_dT1_dT2.repeat(bs,1,1) , L) # shape = (bs, m*N1, m*N2)
        self.covar_pred = covar_dT2.repeat(bs,1,1) - torch.bmm( covar_dT2_dT1.repeat(bs,1,1), self.covar_pred ) # (bs, m*N2, m*N2)

        return self.mean_pred, self.covar_pred



    def sample(self, mean, covar, n_samples=3):
        """
        ...
        samples_std = torch.randn( size=(bs, N, m, n_samples) ) # shape = (bs, N, m, n_samples)

        # Transform to samples from GP.
        var = torch.diagonal(covar, dim1=1, dim2=2).unflatten( dim=1, sizes=(m, N) ).transpose(1,2).unsqueeze(3).repeat(1, 1, 1, n_samples) # shape = (bs, N, m, n_samples)
        mean = mean.unsqueeze(3).repeat(1, 1, 1, n_samples) # shape = (bs, N, m, n_samples)
        samples = mean + (samples_std * torch.sqrt(var) ) # shape = (bs, N, m, n_samples)

        return samples



    def forward(self, T:torch.tensor, dT:torch.tensor, tmax:float, mean_U:torch.tensor, mean_dU:torch.tensor):
        """
        ...
        self.mean_prior = self.mean_module(T, dT, tmax, mean_U, mean_dU, A, B, C, D) # shape = (N, m)
        self.covar_prior = self.covar_module(T, T, dT, dT, tmax, tmax, mean_U, mean_U, mean_dU, mean_dU, A, B, C, D) # shape = (m*N, m*N)
