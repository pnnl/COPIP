...
# Import custom files.
import SEGP
from VAE0 import VAEEncoder, VAEEncoderCNNGN, VAEDecoder, unwrap_angle
...
class ELBO(nn.Module):
    ...
    def kl_divergence(self, mean_post:torch.tensor, covar_post:torch.tensor, 
        ...
        L_post = torch.linalg.cholesky(covar_post, upper=False)

        # Solve covar_post * alpha = covar_prior for alpha.
        alpha = torch.cholesky_solve(covar_post, L_prior.repeat(bs,1,1))
        ...
        alpha = torch.cholesky_solve(mean_post - mean_prior, L_prior) # shape = (bs , m*N, 1).
        ...
        log_det_prior = 2.0 * torch.sum(torch.log(torch.diagonal(L_prior.repeat(bs,1,1), dim1=1, dim2=2)), dim=1)
        log_det_post = 2.0 * torch.sum(torch.log(torch.diagonal(L_post, dim1=1, dim2=2)), dim=1)
        term4 = log_det_prior - log_det_post # (bs,)
...
class LinearScheduler():
    """
    Class for linearly scheduling a value from start_val to end_val over duration epochs.
    """
    def __init__(self, start_val:float, end_val:float, duration:int):
    ...
    def step(self, epoch:int):
        if epoch >= self.duration:
            return self.end_val
        return self.start_val + (self.end_val - self.start_val) * (epoch / self.duration)



def train(T, dT, tmax, mean_U, mean_dU, train_loader, test_loader, max_epoch, enc, GP, dec, optimizer, 
          elbo, kl_weight, model_dir, device, dY_test, ds_vec, us_vec, l1_sched=None, min_epoch=0, stats=None):
    ...
    for epoch in range(min_epoch, max_epoch):
        ...
        for batch, vid_batch in enumerate(train_loader):
            ...
            ##### model outputs #####
            mean_lhood, var_lhood = enc(vid_batch) # both (bs, N, m) where m=2: (r, theta)
            
            # Unwrap theta (dim=1 is time dimension) to avoid discontinuities at ±π
            # mean_lhood[:, :, 0] is r, mean_lhood[:, :, 1] is theta
            mean_lhood_theta_unwrapped = unwrap_angle(mean_lhood[:, :, 1], dim=1)  # (bs, N)
            mean_lhood = torch.stack([mean_lhood[:, :, 0], mean_lhood_theta_unwrapped], dim=2)  # (bs, N, m)
            
            mean_prior, covar_prior = GP(T[:K], dT[:N], tmax, mean_U[:K], mean_dU[:N]) # (N, m) and (m*N, m*N) - same for all items in batch
            ...
            # Add L1 regularization for A matrix to elbo_train
            A_matrix = GP.A  # Assuming GP has an attribute A representing the A matrix
            l1_norm = A_matrix.abs().sum()
            l1_lambda = l1_sched.step(epoch) if l1_sched else 0.0
            loss = elbo_train + l1_lambda * l1_norm
            ...
            batch_l1_train.append(l1_norm.item())
        ...
        if epoch % 10 == 0 or epoch == max_epoch:
            ...
            plot_latents(model_dir + 'Data/Plots', string, vid_batch.unflatten(dim=0, sizes=(1,bs)), dY_test[batch,:,:N,:].unflatten(dim=0, sizes=(1,bs)), dT[:N], tmax, nplots, recon_batch=p_theta, recon_traj=mean_post, recon_covar=covar_post)
        ...
        if epoch == 0:
            stats = pd.DataFrame({
                ...
                'l1 norm':[np.mean(batch_l1_train)]
            })
...
def main():
    ...
    # Use only a fraction of the data for training (to reduce memory usage)
    # Load with mmap_mode to avoid loading entire file into memory
    data_fraction = 1.0  # Use 10% of the data (adjust as needed: 0.75 for 75%, etc.)
    
    # First, get the shape without loading the full array
    vid_batch_mmap = np.load(data_path + '/vid_batch.npy', mmap_mode='r')
    dY_mmap = np.load(data_path + '/dZ.npy', mmap_mode='r')
    
    M_total, Q, N, d, _ = vid_batch_mmap.shape
    M_subset = int(M_total * data_fraction)
    print(f"Loading {data_fraction*100:.0f}% of data: {M_subset} out of {M_total} sequences")
    
    # Load only the subset into memory
    vid_batch = torch.from_numpy(vid_batch_mmap[:M_subset].copy() ).to(device)
    dY = torch.from_numpy( dY_mmap[:M_subset].copy() ).to(device)
    
    # Clean up mmap references
    del vid_batch_mmap, dY_mmap
    
    M = M_subset
    print(f"Using {data_fraction*100:.0f}% of data: M = {M} sequences")
    ...
    total_sequences = M * Q
    if batches * bs != total_sequences:
        if total_sequences % batches == 0:
            bs = total_sequences // batches
            print(f"Adjusted batch size to {bs} so that batches*bs matches total sequences {total_sequences}.")
        elif total_sequences % bs == 0:
            batches = total_sequences // bs
            print(f"Adjusted number of batches to {batches} so that batches*bs matches total sequences {total_sequences}.")
        else:
            common_bs = math.gcd(total_sequences, bs)
            if common_bs == 0:
                raise ValueError("Unable to reconcile dataset size with provided batches and batch size.")
            batches = total_sequences // common_bs
            bs = common_bs
            print(f"Adjusted batches to {batches} and batch size to {bs} (gcd-based) to cover all {total_sequences} sequences without dropping data.")
    ...
    if Load:
        ...
        num_keypoints = model_settings.get('num_keypoints', 1)
        temperature = model_settings.get('temperature', 1.0)
        data_scale = model_settings.get('data_scale', 2.5)
    else:
        m = data_setup['m']
        n = data_setup['n']
        p = data_setup['p']
        ...
        d = data_setup['d']
        h_dim = 500  # Only used by original VAEEncoder, kept for compatibility
        ...
        # CNN encoder settings
        num_keypoints = 1  # Number of spatial keypoints to extract
        temperature = 1.0  # Spatial softmax temperature (lower = sharper attention)
        data_scale = 2.5   # Scale factor based on DataGeneration_Particle.py


    GP = SEGP.SEGP(m, n, p, lt, mean_x0, covar_x0, A, B, C, D).to(device)
    # Use CNN-based encoder with Spatial Softmax and Cartesian-to-Polar conversion
    enc = VAEEncoderCNNGN(d=d, num_keypoints=num_keypoints, temperature=temperature, data_scale=data_scale).to(device)
    ...
    if Load:
        ...
        kl_weight = train_settings['kl_weight']
    else:
        lr = 5e-3 # manually drop lr as needed.
        wd = 1e-5 # weight decay.
        kl_weight = 2.5


    optimizer = torch.optim.AdamW(list(enc.parameters()) + list(dec.parameters()) + list(GP.parameters()),lr=lr, weight_decay=wd)
    elbo = ELBO()
    ...
    model_settings = {'m':m, 'n':n, 'p':p, 'lt':lt, 'mean_x0':mean_x0, 'covar_x0':covar_x0, 
                      ...
                      'num_keypoints':num_keypoints, 'temperature':temperature, 'data_scale':data_scale}
    
    train_settings = {'dataset_number':dataset_number, 'batches':batches, 'bs':bs, 'test_split':test_split, 'seed':seed, 'lr':lr, 'wd':wd, 
            'max_epoch':max_epoch, 'kl_weight':kl_weight}
    ...
    l1_sched = LinearScheduler(2.5e-2, 3e-1, max_epoch)

    # Train model.
    if Load:
        enc, GP, dec = train(T, dT, tmax,  mean_U, mean_dU, train_loader, test_loader, max_epoch, enc, GP, dec, 
                             optimizer, elbo, kl_weight, model_dir, device, dY_test, down_scaling_vec, 
                             up_scaling_vec, l1_sched=l1_sched, min_epoch=min_epoch, stats=stats)
    else:
        enc, GP, dec = train(T, dT, tmax,  mean_U, mean_dU, train_loader, test_loader, max_epoch, enc, GP, dec, 
                             optimizer, elbo, kl_weight, model_dir, device, dY_test, down_scaling_vec, 
                             up_scaling_vec, l1_sched=l1_sched)
